...
void LOS_Complex::init(vector<size_t> _ig, vector<size_t> _jg, vector<complex<double>> _di, vector<complex<double>> _gg, vector<complex<double>> _rp, size_t _n)
{
	ig = _ig;
	jg = _jg;
	di = _di;
...
void LOS_Complex::solve(const vector<complex<double>>& initial_approximation, const size_t maxiter, const double eps, vector<complex<double>>& solution)
{
	if(precond_method > 4)
	{
	...
	for(iteration = 0; iteration < maxiter && !found; iteration++)
	{
		...
		{
			// <double> product_1 = (`wj, zj)
			product_1 = conjugate_prod(w, z);

			// alpha = alphaj = (`wj, rj)/(`wj, zj)
			alpha = conjugate_prod(w, r) / product_1;
			...
			for(size_t i = 0; i < n; i++)
			{
				...
				r[i] -= alpha * z[i];
				s[i] -= alpha * w[i];
				aux[i] = r[i] - sm_r[i];	//for smoothing
			}

			if(smooth)
			{
				eta = -dot_prod_2(sm_r, aux) / dot_prod(aux);
				if(eta < 0.0)
					eta = 0.0;
				else if(eta > 1.0)
					eta = 1.0;
			...
			// beta = betaj = (`wj, aj+1)/(`wj, zj)
			beta = -conjugate_prod(w, a) / product_1;
...
void LOS_Complex::set_smooth(bool value)
{
	smooth = value;
}

string LOS_Complex::create_log_file_name()
{
...
void LOS_Complex::create_general_info()
{
